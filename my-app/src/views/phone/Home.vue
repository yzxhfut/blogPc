...
import HomeBody from '@/components/phone/HomeBody.vue'
import HomeFooter from '@/components/phone/HomeFooter.vue'
  ...
  name: 'home',
	...
	watch: {
		'$route' (newValue) {
			...
			this.articleList = []
			this.$https.get('https://yzxhfut.top:1088/home',{params: 
	...
	created() {
		this.$https.get('https://yzxhfut.top:1088/home',{params: {tag: this.$route.params.tag}}).then(function(res){
            if(res.body.length > 0){
				for (let s of res.body) {
				this.articleList.push(s)
